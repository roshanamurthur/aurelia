...
type OrderStatus = "idle" | "ordering" | "success" | "error";
...
export default function TakeoutOrderButton({ variant = "button" }: TakeoutOrderButtonProps) {
  ...
  const orderTakeout = async () => {
    const bowl = CHIPOTLE_BOWLS[Math.floor(Math.random() * CHIPOTLE_BOWLS.length)];
    ...
    try {
      const res = await fetch("/api/doordash", {
        ...
        body: JSON.stringify({ searchIntent: bowl }),
      });
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-2">
        ...
        {status === "ordering" && (
          <div className="text-center px-2">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-400 font-medium">{selectedBowl}</p>
        ...
        {status === "success" && (
          <div className="text-center px-2">
            ...
            <p className="text-[10px] text-stone-500 mt-0.5 line-clamp-2">{selectedBowl}</p>
            <button type="button" onClick={() => setStatus("idle")} className="text-[10px] text-blue-500 hover:underline mt-1">
              Order another
            </button>
          </div>
        )}
        {status === "error" && (
          <div className="text-center px-2">
            ...
            <button type="button" onClick={orderTakeout} className="text-[10px] text-blue-500 hover:underline mt-1">
              Retry
            </button>
          </div>
        )}
    ...
    <div className="relative inline-flex items-center">
      ...
      {status === "ordering" && (
        <div className="flex items-center gap-3 px-4 py-2 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200/60 dark:border-amber-800/40">
          ...
          <span className="text-sm text-amber-700 dark:text-amber-300 font-medium">{selectedBowl}</span>
          {liveUrl && (
      ...
      {status === "success" && (
        <div className="flex items-center gap-3 px-4 py-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200/60 dark:border-green-800/40">
          <span className="text-sm text-green-600 dark:text-green-400 font-medium">{selectedBowl} added to cart</span>
          <button type="button" onClick={() => setStatus("idle")} className="text-xs text-blue-500 hover:underline">
            Again
          </button>
        </div>
      )}
      {status === "error" && (
        <div className="flex items-center gap-3 px-4 py-2 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200/60 dark:border-red-800/40">
          ...
          <button type="button" onClick={orderTakeout} className="text-xs text-blue-500 hover:underline">
            Retry
          </button>
        </div>
      )}
