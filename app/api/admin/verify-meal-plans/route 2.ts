...
export async function GET() {
  try {
    ...
    const [mealPlans, authUsers] = await Promise.all([
      db
    ...
    const authUsersFormatted = (authUsers as { _id: { toString: () => string }; email?: string; name?: string }[]).map((u) => ({
      userId: String(u._id),
      email: u.email ?? null,
      name: u.name ?? null,
    }));
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        uniqueUserIdsInPlans: [...new Set((mealPlans as { userId: string }[]).map((p) => p.userId))].length,
      },
    });
  } catch (err) {
